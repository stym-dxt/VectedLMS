...
export default function Register() {
  ...
  const sendOtp = async () => {
    if (!auth || !isFirebaseConfigured) {
      setError("Phone sign-up is not configured. Please try again later.");
      return;
    }
  ...
  const verifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await register(phoneForBackend);
      navigate("/dashboard");
    } catch (err: any) {
  ...
  return (
    <AuthLayout
      ...
      footerLinkTo="/login"
    >
      <div ref={recaptchaRef} id="recaptcha-container-register" className="sr-only" aria-hidden="true" />
      ...
      {step === "phone" && (
        <div className="space-y-4">
          <div>
            ...
            />
            <p className="mt-1 text-xs text-slate-500">Include country code. We'll send a one-time code via SMS. You can add your email later in profile.</p>
          </div>
